...
function viewPopup(id) {
        ...
        $.ajax({ 
              type: "GET", 
              url: baseurl+"/dashboard/invoices/preview/"+id,
              success: function(msg){ 
                        var previewHtml = $(msg).find('.content');
                        // var insidePanel = $(previewHtml).html();
                        var srcImg = $("#popupPreview").html(previewHtml);
 ...
 // alert(search);
    ...
    var baseUrlPath=getbaseUrlPath();
     ...
     window.location.href=baseUrlPath+'dashboard/invoices/scheduled-invoices-list'+resultval;
    }
